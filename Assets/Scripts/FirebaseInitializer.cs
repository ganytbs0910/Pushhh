...
public class FirebaseInitializer : MonoBehaviour
{
    ...
    public async UniTask RecordWinning(int winAmount)
    {
        if (!isInitialized)
        {
            Debug.LogError("FirebaseInitializerが初期化されていません");
            return;
        }
    ...
    public async UniTask<int> CalculateWinningAmount()
    {
        ...
        {
            Debug.LogError("FirebaseInitializerが初期化されていません");
            return 1000; // デフォルトの最小当選金額を返す
        }

        try
        {
            await UpdateGlobalSlotCount();
            int calculatedAmount = (int)(globalSlotCount * 0.1f) + 1000;
            return calculatedAmount;
        }
        catch (Exception ex)
        {
            Debug.LogError($"当選金額の計算に失敗しました: {ex.Message}");
            return 1000; // デフォルトの最小当選金額を返す
        }
    }
}